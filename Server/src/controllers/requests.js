...
requestRouter.post("/:requestId", isUser(), async (req, res) => {
    ...
    const request = await getRequestById(requestId).lean();
    const newChat = await acceptRequest(request.sender_id, user._id, requestId);
    res.json({ chat: newChat, request: request });
});
...
requestRouter.delete("/:requestId", isUser(), async (req, res) => {
    ...
    const request = await getRequestById(requestId).lean();
    await declineRequest(receiver._id, requestId);
    res.json(request);
});
